...
A membrane encloses both the information and the actors working on it, not one or the other. An agent, in this reading, is just its harness plus the context available to it on a turn — so an agent and a frame are the same kind of thing, an entity with an activation pattern. Compute that fingerprint over entities and the membrane is whatever it groups: two agents on the same codebase fall inside one naturally, and two people with overlapping research interests are grouped just as organically — close enough on the substrate to be put in touch.

Membranes over shared substrate are an active interest across a few projects, each coming at them from a different direction — from [selectively permeable communication](https://www.julianfleck.net/articles/spirits-dark-forest) to trust-scoped coordination across teams. We mostly want a membrane to do two things at once: **context assembly** — the boundary that decides what context a turn runs on, and at what granularity, since retrieving across the depth of subgraphs sets how coarse or fine it is (see [[fractal-composition]]) — and a **coordination surface** — the boundary through which separate populations of agents align without explicit messaging.
