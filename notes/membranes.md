...
Over the substrate, a membrane is that surface drawn at a moment around a co-active region — the frames and the agents working over them — with sparser coupling outside. What opens its channels is local alignment between frames. How we currently raise that alignment: frames carry an initial phase seeded from their content, and attention events drive them toward each other from there — one mechanism among possible others, and still being worked out. Decay pulls the alignment back down and lets the channels close. A membrane is drawn and released, not a fixed group, and interventions act on the membrane rather than on the agents inside it.

Membranes over shared substrate are an active interest across a few projects, each researching them from a different direction — from [selectively permeable communication](https://www.julianfleck.net/articles/spirits-dark-forest) to trust-scoped coordination across teams. The boundary doesn't separate the information from the actors working on it; it encloses both at once. An agent, in this reading, is just its harness plus the context available to it on a turn — so an agent and a frame are the same kind of thing, an entity with an activation pattern. Compute that fingerprint over entities and the membrane is whatever it groups: two agents working the same codebase fall inside one naturally, and two people with overlapping research interests are grouped just as organically — close enough on the substrate to be put in touch. We are mostly interested in using a membrane for two things at once: **context assembly** — the boundary that decides what context a turn runs on, and at what granularity, since retrieving across the depth of subgraphs sets how coarse or fine that context is (see [[fractal-composition]]) — and a **coordination surface** — the boundary through which separate populations of agents align without explicit messaging.
