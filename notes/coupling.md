...
Valence is what lets the substrate tell productive structure from accreted conflict: two regions can be equally densely coupled and mean opposite things, and a metric blind to valence would read them the same. How valence is assigned — inferred from outcomes, from contradiction signals, or carried explicitly — is still open, and so is whether interference should relax on a different clock than alignment.
...
- How valence is assigned or inferred, and whether the three kinds should build and decay on the same clock.
