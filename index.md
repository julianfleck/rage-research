...
We follow this because agents are increasingly deployed not one at a time but as populations that share memory, context, and retrieval — and what one writes becomes what the others read next. When that arrangement fails, it tends to fail collectively: the group converges too early, a local error gets retrieved back as evidence and amplified, the store drifts without resolving. Oversight that works by reading outputs after the fact is already behind — by the time a single agent's output shows the problem, the others have retrieved and built on it. Our wager is that the medium itself carries earlier signs, readable before any one agent's behaviour expresses them, and that it can be acted on directly: change the substrate the agents draw from, rather than correcting each agent in turn. If that holds, oversight scales with the size and depth of a deployment instead of being outpaced by it.
